...
const transactionTypes = computed(() => {
    let types = [
        ...
        { id: 'credito', label: 'A crédito / pagos', image: '/images/credito.webp', bgColor: '#FFCD87', textColor: '#603814', description: 'Anticipo + Saldo deudor.', disabled: !props.allowCredit || !props.client },
...
<template>
    <Dialog :visible="visible" @update:visible="closeModal" modal header="Procesar pago" class="w-full max-w-4xl mx-4">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-0">
            <PaymentSummaryColumn :payment-mode="paymentMode" :total-amount="totalAmount" :client="client" :transaction-type="transactionType" />

            <div class="lg:col-span-2 p-4 lg:p-8">
                <!-- Selector de Tipo -->
                <div v-if="paymentMode === 'strict' && currentStep === 'selecType'">
                    <h3 class="text-xl font-semibold text-center mb-6 text-gray-800 dark:text-gray-200">¿Cómo deseas registrar esta venta?</h3>
                    ...
                    <div v-if="!client && (allowCredit || allowLayaway)" class="mt-8">
                        ...
                        <div class="flex items-center gap-2 mt-4">
                            ...
                            <Button @click="showCreateCustomerModal = true" rounded icon="pi pi-plus" severity="contrast" size="small" />
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="currentStep === 'processPayment'">
                    <a v-if="paymentMode === 'strict'" @click="currentStep = 'selecType'" class="text-sm text-primary cursor-pointer mb-5 block hover:underline">&larr; Volver</a>
                    <div v-if="isLoadingAccounts" class="flex items-center justify-center min-h-[400px]"><i class="pi pi-spin pi-spinner !text-4xl text-gray-500"></i></div>
                    <MultiPaymentProcessor v-else :total-amount="totalAmount" :client="client" :transaction-type="transactionType" :bank-accounts="bankAccounts" :bank-account-options="bankAccountOptions" :loading="props.loading" @submit="handleSubmitFromProcessor" @add-account="showAddBankAccountModal = true" />
                </div>
            </div>
        </div>
    </Dialog>
