...
const form = useForm({
    ...
    total_discount: props.quote.total_discount,
    total_tax: props.quote.total_tax,
    shipping_cost: props.quote.shipping_cost,
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div class="flex gap-2 mb-4">
                    <Select v-model="selectedItem" :options="availableItems" optionLabel="name" placeholder="Busca un producto o servicio..." filter class="w-full">
                        <template #option="slotProps"><div>{{ slotProps.option.name }} <Tag :value="slotProps.option.type" /></div></template>
                    </Select>
