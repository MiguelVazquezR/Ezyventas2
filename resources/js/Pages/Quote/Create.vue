...
const addItem = () => {
    ...
    const newItem = {
        ...
        variant_details: null, // Para guardar los detalles de la variante
    };

    form.items.push(newItem);

    // Si el producto tiene variantes, abrir el modal
...
watch([() => form.items, () => form.total_discount, () => form.shipping_cost, includeTax, taxType], () => {
    ...
    if (includeTax.value) {
        if (taxType.value === 'added') {
            // El subtotal es la suma de precios base, el impuesto se añade.
            form.subtotal = subtotal;
            tax = subtotalAfterDiscount * taxRate;
        } else { // included
            // El subtotal debe ser la base imponible (el precio sin el IVA ya incluido).
            const baseAmount = subtotalAfterDiscount / (1 + taxRate);
            tax = subtotalAfterDiscount - baseAmount;
            form.subtotal = subtotal - tax; // Ajustamos el subtotal para reflejar el precio base.
        }
    } else {
        form.subtotal = subtotal;
    }
        ...
        <form @submit.prevent="submit" class="mt-6 max-w-4xl mx-auto space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <Select id="customer" v-model="form.customer_id" :options="localCustomers" filter
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div class="flex gap-2 mb-4">
                    <Select v-model="selectedItem" :options="availableItems" optionLabel="name"
                        placeholder="Busca un producto o servicio..." filter class="w-full">
                ...
                <DataTable :value="form.items" class="p-datatable-sm">
                    <Column field="description" header="Descripción">
                        <template #body="{ data, index }">
                            ...
                            <div v-if="data.variant_details" class="text-xs text-gray-500 mt-1">
                                <Button @click="openVariantSelector(index)" label="Cambiar variante" text size="small"
                                    class="!p-0" />
                            </div>
                        </template>
                    </Column>
                    <Column field="quantity" header="Cantidad" style="width: 10rem"><template #body="{ index }">
                            <InputNumber v-model="form.items[index].quantity" class="w-full" showButtons
                                buttonLayout="horizontal" :step="1" decrementButtonClass="p-button-secondary"
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <InputLabel for="shipping_address" value="Dirección de Envío" /><Textarea id="shipping_address"
                            v-model="form.shipping_address" rows="3" class="mt-1 w-full" />
                    </div>
                    <div class="md:col-span-2">
                        <InputLabel for="notes" value="Notas Adicionales" /><Textarea id="notes" v-model="form.notes"
                            rows="3" class="mt-1 w-full" />
                    </div>
                </div>
            </div>
