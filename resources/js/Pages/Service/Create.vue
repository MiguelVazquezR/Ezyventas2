...
import ToggleSwitch from 'primevue/toggleswitch'; // Aseguramos importar el Toggle
...
// --- ACTUALIZACIÓN: Estructura del formulario con variantes ---
const form = useForm({
    name: '',
    description: '',
    category_id: null,
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <template v-if="!form.has_variants">
                    <div>
                        ...
                        <InputNumber id="base_price" v-model="form.base_price" mode="currency" currency="MXN" locale="es-MX" class="w-full mt-1" placeholder="$0.00" />
                ...
                <!-- Imagen -->
                 <div class="md:col-span-2 mt-4">
                    ...
                    <FileUpload @select="form.image = $event.files[0]" :auto="true" :customUpload="true" accept="image/*" :show-upload-button="false" :show-cancel-button="false">
                        <template #empty><p>Arrastra y suelta una imagen aquí o haz clic para seleccionar.</p></template>
                    </FileUpload>
                 </div>
            </div>
            
            <div class="flex justify-end mt-8 pt-4 border-t dark:border-gray-700">
                <Button type="submit" label="Guardar Servicio" icon="pi pi-save" :loading="form.processing" severity="primary" class="w-full md:w-auto" />
            </div>
        </form>
