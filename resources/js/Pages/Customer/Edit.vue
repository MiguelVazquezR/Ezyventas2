...
const mainAddr = props.customer.address || emptyAddress();
const fiscalAddr = props.customer.fiscal_address || null;

// Detect if fiscal address matches main address (or is empty)
const sameAddress = ref(!fiscalAddr || Object.values(fiscalAddr).every(v => !v));
...
const form = useForm({
    ...
    tax_regime: props.customer.tax_regime || '',
    ...
    <AppLayout :title="`Editar cliente: ${customer.name}`">
        ...
        <form @submit.prevent="submit" class="mt-6 max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
            ...
            <Panel v-if="hasBilling" header="Datos para Facturación" :toggleable="true" :collapsed="false" class="mb-6 !rounded-2xl !border !border-gray-200" :pt="{ header: { class: '!bg-gray-50 !rounded-t-2xl !py-3 !px-4 !text-sm !font-semibold !text-gray-700 !border-none' }, content: { class: '!bg-white !p-4 !rounded-b-2xl' }, toggler: { class: '!text-gray-500' } }">
                ...
                <!-- Fiscal address fields (always editable) -->
                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                    <div class="md:col-span-4">
            ...
            <!-- SECCIÓN 4: CONFIGURACIÓN FINANCIERA -->
            <div class="mb-6">
