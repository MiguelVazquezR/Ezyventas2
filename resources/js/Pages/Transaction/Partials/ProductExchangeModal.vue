...
const props = defineProps({
    visible: Boolean,
    transaction: Object,
    userBankAccounts: { type: Array, default: () => [] }, // <--- NUEVA PROP
});
...
const selectedBankAccountId = ref(null); // <--- NUEVO: Para cuenta bancaria

// --- NUEVO: Gestión de Cliente y Reembolso ---
const showCreateCustomerModal = ref(false);
const assignedCustomer = ref(null);
const exchangeRefundType = ref('cash'); // 'cash' o 'balance'
...
// --- Lógica Paso 3 (Nuevos Productos) ---
...
// --- Lógica Paso 4 (Resumen y Pago) ---
const balanceDifference = computed(() => {
    return totalNewSaleAmount.value - totalRefundAmount.value;
});
...
const canUseCredit = computed(() => {
    return assignedCustomer.value && availableCredit.value >= balanceDifference.value;
});
...
const form = useForm({
    ...
    use_credit_for_shortage: false, // <--- NUEVO: Flag para indicar crédito
});
...
const submitExchange = () => {
    ...
    if (balanceDifference.value > 0.01 && paymentMethod.value === 'credito') {
        ...
        // Opcional: Validar límite de crédito estricto aquí o dejar que el backend lo haga
    }

    // Validación de Cuenta Bancaria
    if (balanceDifference.value > 0.01 && ['tarjeta', 'transferencia'].includes(paymentMethod.value) && !selectedBankAccountId.value) {
    ...
    if (balanceDifference.value > 0.01) {
        if (paymentMethod.value === 'credito') {
            // Si es crédito, NO enviamos pago, activamos flag
            useCredit = true;
        } else {
            ...
            paymentsPayload.push({
                ...
                bank_account_id: selectedBankAccountId.value, // Enviamos la cuenta seleccionada
            });
        }
    }
    ...
    form.use_credit_for_shortage = useCredit; // <--- Enviamos flag
...
<template>
    <Dialog :visible="visible" @update:visible="emit('update:visible', $event)" modal header="Asistente de intercambio"
        :style="{ width: '55rem' }" :breakpoints="{ '960px': '85vw', '641px': '100vw' }">
            ...
            <Stepper v-model:value="currentStepValue" linear class="w-full">
                ...
                <StepPanels>
                    ...
                    <StepPanel v-slot="{ activateCallback }" value="4">
                        <div class="flex flex-col min-h-[350px]">
                            <h4 class="font-bold mb-4 text-center text-xl text-gray-800 dark:text-white">Resumen del
                                intercambio</h4>

                            <div class="grid grid-cols-2 gap-4 mb-6">
                                <div
                                    class="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg text-center border border-green-200 dark:border-green-800">
                                ...
                                <div
                                    class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-center border border-blue-200 dark:border-blue-800">
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Nueva venta</p>
                                    <p class="text-xl font-bold text-blue-700 dark:text-blue-400">+ {{
                                        formatCurrency(totalNewSaleAmount) }}
                                    </p>
                                </div>
                            </div>

                            <div class="text-center mb-6 py-4 rounded-xl"
                                :class="balanceDifference > 0 ? 'bg-red-50 dark:bg-red-900/10' : 'bg-green-50 dark:bg-green-900/10'">
                                <p class="text-lg font-medium text-gray-600 dark:text-gray-400">Diferencia final:</p>
                                <p class="text-4xl font-black my-1"
                                    :class="balanceDifference > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'">
                                    {{ formatCurrency(Math.abs(balanceDifference)) }}
                                </p>
                                <p class="text-sm font-bold tracking-wider uppercase"
                                    :class="balanceDifference > 0 ? 'text-red-500' : 'text-green-500'">
                                    {{ balanceDifference > 0 ? 'EL CLIENTE DEBE PAGAR' : 'SALDO A FAVOR DEL CLIENTE' }}
                                </p>
                            </div>

                            <!-- Caso: SOBRA DINERO (Balance Negativo) - Sin cambios aquí -->
                            <div v-if="balanceDifference < -0.01">
                                <!-- ... (Lógica de devolución / Saldo) ... -->
                                <div v-if="assignedCustomer"
                                    class="bg-green-100 dark:bg-green-900/30 p-4 rounded-lg border border-green-300 dark:border-green-700 text-green-800 dark:text-green-300 flex flex-col gap-3 mb-4">
                                    <div class="flex items-center gap-3">
                                        <i class="pi pi-check-circle text-2xl"></i>
                                        <div>
                                            <p class="font-bold m-0">Cliente identificado: {{ assignedCustomer.name }}</p>
                                            <p class="text-sm opacity-90 m-0">El excedente se abonará a su saldo a favor.
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div v-else
                                    class="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg border border-orange-200 dark:border-orange-800 mb-4">
                                    <div class="flex items-start gap-3 mb-4">
                                        <i class="pi pi-exclamation-triangle text-orange-500"></i>
                                        <div>
                                            <p class="font-bold text-orange-700 dark:text-orange-300">Cliente no
                                                registrado</p>
                                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                                Para guardar este dinero como saldo a favor, registra un nuevp cliente, de lo
                                                contrario <b>deberás devolver el efectivo al cliente</b> ya que se registrará un movimiento
                                                de salida de caja.
                                            </p>
                                        </div>
                                    </div>
                                    <div class="flex flex-col gap-3">
                                        <div
                                            class="flex items-center gap-4 bg-white dark:bg-gray-800 p-2 rounded border dark:border-gray-700">
                                            <div class="flex items-center">
                                                <RadioButton v-model="exchangeRefundType" inputId="refundTypeCash"
                                                    name="refundType" value="cash" /><label for="refundTypeCash"
                                                    class="ml-2 text-sm cursor-pointer">Devolver
                                                    efectivo</label>
                                            </div>
                                            <div class="flex items-center">
                                                <RadioButton v-model="exchangeRefundType" inputId="refundTypeBalance"
                                                    name="refundType" value="balance" :disabled="!assignedCustomer" />
                                                <label for="refundTypeBalance" class="ml-2 text-sm cursor-pointer"
                                                    :class="{ 'text-gray-400': !assignedCustomer }">Abonar
                                                    a Saldo</label>
                                            </div>
                                        </div>
                                        <Button label="Registrar nuevo cliente" icon="pi pi-user-plus"
                                            severity="primary" @click="showCreateCustomerModal = true"
                                            class="w-full" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-if="balanceDifference > 0.01"
                                class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm mb-4">
                                <h5
                                    class="font-bold mb-3 text-sm text-gray-700 dark:text-gray-200 border-b pb-2 dark:border-gray-600">
                                    Registrar pago de diferencia</h5>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <InputLabel value="Método de pago" class="mb-1" />
                                        <!-- Modificamos opciones para incluir 'credito' y usar label/value -->
                                ...
                                <!-- Alerta para Crédito -->
                                <div v-if="paymentMethod === 'credito'" class="mt-3">
                                    <div v-if="!assignedCustomer" class="text-red-500 text-sm flex items-center gap-2">
                                        <i class="pi pi-times-circle"></i>
                                        <span>No se puede dejar a crédito a público general.</span>
                                    </div>
                                    <div v-else-if="availableCredit < balanceDifference"
                                        class="text-orange-500 text-sm flex items-center gap-2">
                                        <i class="pi pi-exclamation-circle"></i>
                                        <span>Límite de crédito excedido (Disponible: {{ formatCurrency(availableCredit)
                                        }}).</span>
                                    </div>
                                    ...
                                    <Select v-model="selectedBankAccountId" :options="userBankAccounts"
                                        ...
                                        <template #option="slotProps">
                                            <div class="flex flex-col">
                                                ...
                                                <span class="text-xs text-gray-500">{{ slotProps.option.bank_name }} -
                                                    ...{{ slotProps.option.account_number.slice(-4) }}</span>
                                            </div>
                                        </template>
                                    </Select>
                                </div>
    ...
    <!-- Modal para crear cliente si no existe y queremos abonar saldo -->
