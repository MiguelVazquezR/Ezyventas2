...
import PaymentModal from '@/Components/PaymentModal.vue'; // NUEVO
import StartSessionModal from '@/Components/StartSessionModal.vue'; // NUEVO
import JoinSessionModal from '@/Components/JoinSessionModal.vue'; // NUEVO
...
const sessionModalAwaitingPayment = ref(false); // Para reabrir pago tras abrir caja

watch(activeSession, (newSession) => {
    if (newSession && sessionModalAwaitingPayment.value) {
        sessionModalAwaitingPayment.value = false;
        openPaymentModal(); // Reintentar abrir modal de pago
    }
});
...
const openPaymentModal = () => {
    ...
    if (!activeSession.value) {
        // Lógica "inteligente" para ofrecer abrir caja
...
const actionItems = computed(() => [
    // Opción para abonar (Nueva)
    { label: 'Abonar / Liquidar', icon: 'pi pi-dollar', command: openPaymentModal, disabled: !canAddPayment.value, visible: hasPermission('transactions.add_payment') }, // hasPermission('transactions.add_payment')
    { separator: true },
...
<template>
    <AppLayout :title="`Venta #${transaction.folio}`">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                <Card>
                    ...
                    <template #content>
                        <ul class="space-y-3 text-sm">
                            ...
                            <li v-if="parseFloat(transaction.total_discount) > 0" class="flex justify-between"><span>Descuento:</span><span class="text-green-500">- {{ formatCurrency(transaction.total_discount) }}</span></li>
                            <li class="flex justify-between font-bold text-base border-t pt-2 mt-2"><span>Total:</span><span>{{ formatCurrency(totalAmount) }}</span></li>
                        ...
                        <!-- Botón directo para abonar si hay deuda -->
                        ...
                        <ul class="space-y-3 text-sm">
                            ...
                            <li class="flex justify-between items-center">
                                ...
                                <span class="font-medium">
                                    <Link v-if="transaction.customer" :href="route('customers.show', transaction.customer.id)" class="text-blue-600 hover:underline flex items-center gap-2">{{ transaction.customer.name }} <i class="pi pi-external-link text-xs"></i></Link>
                ...
                <Card>
                    ...
                    <template #content>
                        <div v-if="!localTransaction.payments?.length"><p class="text-center text-gray-500 text-sm py-4">No se han registrado pagos.</p></div>
        ...
        <Dialog v-model:visible="isRefundModalVisible" modal header="Confirmar devolución" :style="{ width: '30rem' }">
            <div class="p-fluid">
                ...
                <div class="flex flex-col gap-3">
                    <div v-if="props.transaction.customer_id" class="flex items-center"><RadioButton v-model="refundMethod" inputId="refundBalance" value="balance" /><label for="refundBalance" class="ml-2">Abonar al saldo del cliente</label></div>
                    <div class="flex items-center"><RadioButton v-model="refundMethod" inputId="refundCash" value="cash" :disabled="!activeSession" /><label for="refundCash" class="ml-2">Retirar efectivo de caja</label><small v-if="!activeSession" class="ml-2 text-orange-500">(Sin sesión activa)</small></div>
                </div>
            </div>
            <template #footer>
        ...
        <!-- NUEVO: Modal de Pagos (Para abonos) -->
        <!-- Usamos mode='flexible' para permitir pago parcial (abono) -->
        <PaymentModal
            v-if="isPaymentModalVisible" 
            v-model:visible="isPaymentModalVisible"
            :total-amount="totalAmount"
            :amount-to-pay="pendingAmount" 
        ...
        <!-- Modales de Sesión (Para cuando no hay caja abierta) -->
