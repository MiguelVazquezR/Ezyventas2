                                ...
                                <div v-for="setting in moduleSettings" :key="setting.id" class="flex items-start justify-between">
                                    ...
                                    <div class="flex-shrink-0 ml-4 w-64">
                                        ...
                                        <div v-if="setting.type === 'file'" class="w-full">
                                            ...
                                            <a v-if="typeof form.settings[setting.key] === 'string' && form.settings[setting.key] && !selectedFiles[setting.key]" :href="form.settings[setting.key]" target="_blank" class="text-sm text-sky-600 hover:underline mb-2 block truncate" :title="form.settings[setting.key]"> Ver archivo actual</a>
                                            
                                            <!-- Muestra el nombre del archivo nuevo que se acaba de seleccionar -->
                                            <div v-if="selectedFiles[setting.key]" class="text-sm text-gray-600 dark:text-gray-400 mb-2 flex items-center justify-between bg-gray-100 dark:bg-gray-700 p-2 rounded-md">
                                                <span class="truncate" :title="selectedFiles[setting.key].name">{{ selectedFiles[setting.key].name }}</span>
                                                <Button icon="pi pi-times" @click="clearFileSelection(setting.key, setting.value)" text rounded class="h-6 w-6 !text-red-500" />
                                            </div>

                                            <FileUpload mode="basic" :name="`settings[${setting.key}]`" @select="handleFileSelect($event, setting.key)" :auto="true" customUpload :chooseLabel="form.settings[setting.key] && typeof form.settings[setting.key] === 'string' ? 'Cambiar archivo' : 'Elegir archivo'" class="p-button-sm p-button-outlined" />
                                        </div>
