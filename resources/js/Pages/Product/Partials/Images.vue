...
<template>
    <div id="images" class="bg-white dark:bg-[#232323] p-6 rounded-3xl border border-gray-100 dark:border-[#3a3a3a] scroll-mt-24">
        ...
        <div>
            ...
            <div v-if="existingGeneralImages.length > 0" class="flex flex-wrap gap-4 mb-4">
                <div v-for="img in existingGeneralImages" :key="img.id" class="relative group w-24 h-24">
                    <img :src="img.original_url" class="w-full h-full object-cover rounded-xl border border-gray-100 dark:border-[#3a3a3a]">
            ...
            <FileUpload 
                ...
                class="border border-gray-100 dark:border-[#3a3a3a] rounded-2xl"
            >
                <template #empty>
        ...
        <div v-if="attributesRequiringImages.length > 0 && requiredVariantImagesList.length > 0" class="mt-8 pt-6 border-t border-gray-100 dark:border-[#3a3a3a]">
            
            <div class="mb-5 bg-blue-50 dark:bg-blue-900/10 p-4 rounded-2xl border border-blue-100 dark:border-blue-900/30 flex gap-3 items-start">
                <i class="pi pi-camera text-blue-500 !text-xl mt-0.5"></i>
                <div>
                    <h5 class="font-bold text-blue-800 dark:text-blue-300 m-0">Fotografías específicas requeridas</h5>
                    <p class="text-xs text-blue-600 dark:text-blue-400 m-0 mt-1">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                <div v-for="item in requiredVariantImagesList" :key="item.key" class="flex flex-col gap-2">
                    ...
                    <div class="bg-gray-100 dark:bg-[#1a1a1a] text-center py-1.5 px-2 rounded-xl font-semibold text-xs text-gray-700 dark:text-gray-300 truncate" :title="item.value">
                        {{ item.attribute }}: <span class="text-blue-600 dark:text-blue-400">{{ item.value }}</span>
                    </div>

                    <div class="relative w-full aspect-square border-2 rounded-2xl overflow-hidden transition-all group"
                        :class="previews[item.key] || getExistingVariantImage(item.key) ? 'border-gray-100 dark:border-[#3a3a3a]' : 'border-dashed border-gray-200 dark:border-[#3a3a3a] hover:bg-gray-50 dark:hover:bg-[#1a1a1a] cursor-pointer'">
