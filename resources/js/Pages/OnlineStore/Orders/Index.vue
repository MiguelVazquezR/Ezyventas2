    ...
    <AppLayout>
        <div class="p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6">
            <div class="bg-white dark:bg-[#232323] p-6 lg:p-8 rounded-3xl border border-gray-100 dark:border-[#3a3a3a]">
                <div class="mb-8">
                    <h1 class="text-3xl md:text-4xl font-light tracking-tight text-gray-900 dark:text-white m-0">Pedidos</h1>
                    <p class="text-[10px] uppercase tracking-widest font-bold text-gray-500 m-0 mt-2 flex items-center gap-2">
                        ...
                        {{ totalPending > 0 ? `${totalPending} pedido(s) pendiente(s)` : 'Gestiona los pedidos de tu tienda' }}
                    </p>
                </div>
                ...
                <DataTable :value="orders.data" paginator :rows="20" :totalRecords="orders.total" class="w-full"
                    ...
                    <Column field="formatted_order_number" header="Pedido #">
                        <template #body="{ data }">
                    ...
                    <Column field="delivery_type" header="Tipo">
                        <template #body="{ data }">
                            <Tag :value="data.delivery_type === 'pickup' ? 'Recoger' : 'Envío'" :severity="data.delivery_type === 'pickup' ? 'info' : 'warn'" class="!text-[10px] !uppercase !tracking-widest !font-bold" />
                        </template>
                    </Column>
