...
// Inicializar el datepicker con las fechas del controlador
onMounted(() => {
        ...
        <div class="p-4 md:p-6 lg:p-8 space-y-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <Button @click="setDateRange('this_week')" label="Esta Semana" text size="small" />
                    <Button @click="setDateRange('this_month')" label="Este Mes" text size="small" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card>
                    ...
                    <template #subtitle>Periodo seleccionado</template>
                    <template #content><p class="text-2xl font-bold text-green-600">{{ formatCurrency(kpis.totalIncome) }}</p></template>
                </Card>
                 <Card>
                    <template #title>Gastos</template>
                    <template #subtitle>Periodo seleccionado</template>
                    <template #content><p class="text-2xl font-bold text-red-600">{{ formatCurrency(kpis.totalExpenses) }}</p></template>
                </Card>
                 <Card>
                    <template #title>Beneficio Neto</template>
                    <template #subtitle>Periodo seleccionado</template>
                    <template #content><p class="text-2xl font-bold" :class="kpis.netProfit >= 0 ? 'text-gray-800 dark:text-gray-200' : 'text-red-600'">{{ formatCurrency(kpis.netProfit) }}</p></template>
                </Card>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                    <Card class="h-full">
                        ...
                        <template #content>
                            <Chart type="line" :data="chartDataFormatted" :options="chartOptions" class="h-96" />
                        </template>
                    </Card>
                </div>
            ...
            <Card>
                ...
                <template #content>
                    <DataTable :value="recentSessions" class="p-datatable-sm">
                        ...
                        <Column field="calculated_cash_total" header="Total Calculado"><template #body="{data}">{{ formatCurrency(data.calculated_cash_total) }}</template></Column>
