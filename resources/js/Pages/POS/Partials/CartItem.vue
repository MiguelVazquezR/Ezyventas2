...
<template>
    <div class="flex gap-4 relative bg-white dark:bg-gray-900 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
         <!-- SOLUCIÓN 1: Enviar el identificador único del carrito (cartItemId) -->
         <Button @click="$emit('removeItem', item.cartItemId)" icon="pi pi-trash" rounded text severity="danger" size="small" class="absolute top-1 right-1"/>
        ...
        <div class="flex-grow">
            ...
            <div v-if="isEditingPrice" class="flex items-center gap-2">
                 <InputText v-model.number="price" mode="decimal" :minFractionDigits="2" :maxFractionDigits="2" class="p-inputtext-sm w-24" @keyup.enter="applyPriceChange" @keyup.esc="cancelPriceEdit"/>
            ...
            <p class="text-xs text-gray-500" v-if="item.variants && Object.keys(item.variants).length > 0">
